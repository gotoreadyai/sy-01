...
import { RES_LIST } from "../maintenances";
...
export const VehiclesShow = () => {
  ...
  const maintQuery = useList({
    resource: RES_LIST,
    filters: [{ field: "asset_id", operator: "eq", value: record.asset_id }],
    sorters: [{ field: "due_at", order: "asc" }],
    pagination: { current: 1, pageSize: 100 },
  });
  const maints = (maintQuery.data?.data as any[]) ?? [];
  const nextDue = maints.filter((m) => !m.done_at && m.due_at).sort((a,b)=>+new Date(a.due_at)-+new Date(b.due_at))[0];
    ...
    <SubPage>
      ...
      <FlexBox>
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => edit("vehicles", record.id)}><Edit className="w-4 h-4 mr-2" /> Edytuj</Button>
          <Button variant="outline" onClick={() => navigate(`/admin/maintenances/create?asset_id=${record.asset_id}&kind=usterka`)}>
            <Wrench className="w-4 h-4 mr-2" /> Zgłoś usterkę
          </Button>
          <Button variant="outline" onClick={() => navigate(`/admin/maintenances/create?asset_id=${record.asset_id}`)}>
      ...
      <Card className="mt-4">
        ...
        <CardContent className="space-y-4">
          <div className="text-sm"><b>Następny przegląd:</b> {nextDue?.due_at ? new Date(nextDue.due_at).toLocaleDateString() : "—"}</div>
          <Separator />
          <div>
                  ...
                  {activeIssues.map((i) => (
                    <div key={i.service_id} className="border rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button size="sm" variant="outline" onClick={() => navigate(`/admin/maintenances/edit/${i.service_id}`)}>
                          <CheckCircle className="w-4 h-4 mr-2" /> Zakończ
                        </Button>
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">Zgłoszono: {i.created_at ? new Date(i.created_at).toLocaleString() : "—"}</div>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={() => navigate(`/admin/maintenances/create?asset_id=${record.asset_id}&kind=naprawa`)}>
                <Plus className="w-4 h-4 mr-2" /> Dodaj wpis
              </Button>
            </div>
              ...
              : <div className="space-y-3 mt-2">
                  {historyRepairs.map((h) => (
                    <div key={h.service_id} className="border rounded-lg p-3">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">{h.done_at ? new Date(h.done_at).toLocaleDateString() : "—"}</div>
