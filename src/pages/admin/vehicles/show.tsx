...
export const VehiclesShow = () => {
  ...
  // ⚠️ Hook musi być wywołany przed jakimkolwiek warunkowym return
  const assetId = (queryResult?.data?.data as any)?.asset_id as number | undefined;

  const maintQuery = useList({
    resource: RES_LIST,
    filters: assetId ? [{ field: "asset_id", operator: "eq", value: assetId }] : [],
    sorters: [{ field: "due_at", order: "asc" }],
    pagination: { current: 1, pageSize: 100 },
    // Nie odpalaj zapytania dopóki nie mamy assetId:
    ...
    <SubPage>
      ...
      <FlexBox>
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={() => navigate(`/admin/maintenances/create?asset_id=${record.asset_id}&kind=usterka`)}
          >
            <Wrench className="w-4 h-4 mr-2" /> Zgłoś usterkę
          </Button>
          <Button
            variant="outline"
            onClick={() => navigate(`/admin/maintenances/create?asset_id=${record.asset_id}`)}
          >
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="text-sm space-y-1">
            ...
            {record.created_at && (
              <div>
                <b>Utworzono:</b> {new Date(record.created_at).toLocaleString()}
              </div>
            )}
            {record.updated_at && (
              <div>
                <b>Aktualizacja:</b> {new Date(record.updated_at).toLocaleString()}
              </div>
            )}
          </div>
        </CardContent>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                variant="outline"
                onClick={() =>
                  navigate(`/admin/maintenances/create?asset_id=${record.asset_id}&kind=naprawa`)
                }
              >
                <Plus className="w-4 h-4 mr-2" /> Dodaj wpis
              </Button>
