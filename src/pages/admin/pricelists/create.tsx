...
export const PricelistsCreate = () => {
  ...
  const { refineCore: { onFinish }, register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } =
    useForm({ refineCoreProps: { resource: "pricelists" } });
  ...
  return (
    <SubPage>
      ...
      <FlexBox><Lead title="Nowa wersja cennika" description="Utwórz cennik (rich-text)" /></FlexBox>
        ...
        <CardContent>
          <Form onSubmit={handleSubmit(submit)}>
            ...
            <FormControl label="Treść cennika (rich-text)">
              <Textarea rows={10} {...register("content")} placeholder="Wklej/napisz treść cennika. Obsługa RTE do podmiany w kolejnym kroku." />
            </FormControl>
            ...
            <FormActions>
              <Button type="button" variant="outline" onClick={() => list("pricelists")}>Anuluj</Button>
