...
export const BackofficeMenu: React.FC<BackofficeMenuProps> = ({ onClose, brand = "ScrapYard Manager" }) => {
  ...
  return (
    <div className="flex h-full w-64 flex-col border-r bg-background">
      ...
      <Separator />
      <div className="p-4 text-xs text-muted-foreground">Zalogowano jako: <span className="font-medium">{user?.email}</span></div>
      <div className="p-4">
        <Button variant="outline" size="sm" onClick={() => { logout(); onClose?.(); }} className="w-full justify-start">
